...
const VERCEL_CONFIG_PATH = join(ROOT, "vercel.json");
const MANIFEST_PATH = join(DIST_DIR, ".vite", "manifest.json");

// Priority tag pages generated by BlogTag.jsx — keep in sync with TAG_META in src/pages/BlogTag.jsx
...
async function readRedirectedPostSlugs() {
  try {
    const config = JSON.parse(await readFile(VERCEL_CONFIG_PATH, "utf8"));
    return new Set(
      (config.redirects || [])
        .map((redirect) => redirect?.source?.match(/^\/blog\/([^:*]+)$/)?.[1])
        .filter(Boolean)
    );
  } catch (e) {
    console.warn("⚠️ Impossible de lire les redirections Vercel :", e?.message);
    return new Set();
  }
}
...
async function build() {
  ...
  // 4) Tags blog
  const tagsXml = tagSlugs.map((slug) =>
    urlNode({
      loc: abs(`/blog/tag/${slug}`),
      lastmod: today,
      changefreq: "weekly",
      priority: PRIORITY_TAG_SLUGS.includes(slug) ? "0.6" : "0.4",
    })
  );
...
`,
  ...
  for (const m of postsJs.matchAll(
    ...
    const slug = m[1];
    if (redirectedPostSlugs.has(slug)) continue;
    const varName = m[2];
