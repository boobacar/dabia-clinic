...
export default function LocaliteSacreCoeur() {
  ...
  const jsonLd = [
    { "@context": "https://schema.org", "@type": "LocalBusiness", name: "Clinique Dentaire DABIA – Sacré‑Cœur (proche)", url, image: "/og-image.jpg", telephone: "+221777039393", address: { "@type": "PostalAddress", streetAddress: "Sicap Foire, 2 voies Liberté 6 (à 150m du Uno)", addressLocality: "Dakar", addressCountry: "SN" }, areaServed: ["Sacré‑Cœur", "Mermoz", "Point E"], openingHours: ["Mo-Fr 09:00-18:00", "Sa 09:00-14:00"], priceRange: "CFA" },
  ];

  return (
    <section className="py-20 px-4 max-w-6xl mx-auto mt-20">
      <Seo title="Dentiste à Sacré‑Cœur (proche) – Clinique DABIA" description="Dentiste proche de Sacré‑Cœur : prévention, urgences, esthétique, orthodontie et implants. Accès simple depuis Sicap Foire." url={url} canonical={url} jsonLd={jsonLd} />
      <Breadcrumbs items={[{ label: "Accueil", href: "/" }, { label: "Dentiste Sacré‑Cœur" }]} />
      <motion.h1 className="text-3xl md:text-4xl font-bold text-[#ad9d64]" initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}>Dentiste proche de Sacré‑Cœur</motion.h1>
      <motion.p className="mt-3 text-gray-700" initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.1, duration:0.4 }}>Prise en charge complète et technologies modernes pour votre confort.</motion.p>
