// src/data/technologies.js
// Données partagées entre la liste des technologies et la page détail

import radioPan from "../assets/technologie/tech-radio.webp";
import teleradio from "../assets/technologie/tech-teleradio.webp";
import camera from "../assets/technologie/tech-camera.webp";
import cbct from "../assets/technologie/tech-cbct.webp";
import laser from "../assets/technologie/tech-laser.webp";
import scanner from "../assets/technologie/tech-scanner.webp";
import aspiration from "../assets/technologie/tech-aspiration.webp";
import autoclave from "../assets/technologie/tech-autoclave.webp";
import ia from "../assets/technologie/tech-ia.webp";
import led from "../assets/technologie/tech-led.webp";

export const technologies = [
  {
    slug: "radiographie-panoramique-numerique",
    titre: "Radiographie panoramique numérique",
    description:
      "Vue complète des mâchoires en un cliché, dose réduite et diagnostic rapide.",
    image: radioPan,
    details:
      "Cette technologie capture une image complète de votre bouche en une seule prise : dents, mâchoires, articulations, sinus et nerfs. Grâce à un capteur numérique et une rotation douce autour de votre tête, elle remplace les radiographies traditionnelles par une solution plus sûre (dose réduite) et plus efficace.",
  },
  {
    slug: "teleradiographie-cephalometrique",
    titre: "Téléradiographie céphalométrique",
    description:
      "Analyse précise des rapports squelettiques pour les traitements orthodontiques.",
    image: teleradio,
    details:
      "Radiographie latérale du crâne à faible dose qui montre l’ensemble de la structure faciale (mâchoires, dents, ATM, tissus mous) pour élaborer un plan orthodontique individualisé.",
  },
  {
    slug: "camera-intra-orale",
    titre: "Caméra intra-orale",
    description:
      "Visualisation HD en temps réel de l'intérieur de la bouche sur écran.",
    image: camera,
    details:
      "Il s’agit d’une micro-caméra haute résolution, miniature et indolore, qui capture des images précises de l’intérieur de votre bouche. Elle nous permet de visualiser chaque détail de vos dents, gencives et muqueuses, même dans les zones difficiles d’accès.",
  },
  {
    slug: "cone-beam-ct-cbct",
    titre: "Cone Beam CT (CBCT)",
    description: "Imagerie 3D ultra-précise pour implants et chirurgie guidée.",
    image: cbct,
    details: `Cet équipement révolutionnaire capture des images 3D détaillées de vos dents, mâchoires, nerfs et structures osseuses en quelques secondes.

Dans quels cas l’utilisons-nous ?
- Planification d’implants dentaires
- Analyse de dents incluses
- Diagnostic de pathologies osseuses ou sinusiennes
- Traitements orthodontiques complexes
- Chirurgie buccale`,
  },
  {
    slug: "laser-dentaire",
    titre: "Laser dentaire",
    description:
      "Traitements des tissus mous quasi indolores et sans saignement.",
    image: laser,
    details:
      "Précision inégalée, confort optimal, guérison accélérée, sécurité et sérénité pour des traitements des tissus mous et certaines chirurgies en douceur.",
  },
  {
    slug: "scanner-intra-oral-3d",
    titre: "Scanner intra-oral 3D",
    description: "Empreintes numériques et prothèses sur mesure sans pâte.",
    image: scanner,
    details: `Grâce à une petite caméra haute définition, nous capturons en quelques minutes une image 3D ultra-précise de votre bouche. Sans pâte désagréable, sans gouttière, et sans rayonnement.

Utilisations : planification d’implant, couronnes/bridges/facettes, orthodontie (aligneurs), suivi de l’usure dentaire.`,
  },
  {
    slug: "aspiration-chirurgicale",
    titre: "Aspiration chirurgicale",
    description:
      "Système haute puissance pour un champ opératoire propre et sécurisé.",
    image: aspiration,
    details: `Pendant vos soins dentaires, l’aspiration chirurgicale élimine en temps réel :
- Les résidus de salive, de sang ou de tissus
- Les liquides d’irrigation
- Les aérosols
Pour un confort optimisé, une sécurité renforcée et une visibilité parfaite.`,
  },
  {
    slug: "autoclaves-modernes-classe-b",
    titre: "Autoclaves modernes (classe B)",
    description:
      "Stérilisation conforme aux normes européennes pour instruments impeccables.",
    image: autoclave,
    details:
      "Nous utilisons un autoclave de Classe B (norme EN 13060) pour garantir une stérilisation de niveau hospitalier et une hygiène irréprochable de tous les instruments.",
  },
  {
    slug: "intelligence-artificielle-ia",
    titre: "Intelligence artificielle (IA)",
    description:
      "Aide au diagnostic radiologique et planification de traitements complexes.",
    image: ia,
    details:
      "L’IA analyse des données dentaires pour repérer des détails subtils sur vos radios, anticiper les besoins et personnaliser chaque traitement en soutien de l’expertise clinicienne.",
  },
  {
    slug: "blanchiment-par-lampe-led",
    titre: "Blanchiment par lampe LED",
    description:
      "Sourire éclatant en une séance grâce à une lumière froide sécurisée.",
    image: led,
    details:
      "Activation rapide du gel blanchissant sans chauffer la pulpe ; jusqu’à 8 teintes plus claires en 45 minutes.",
  },
];

export default technologies;

